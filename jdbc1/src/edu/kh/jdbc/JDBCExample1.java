...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		 ...
		 * - Java 에서 DB에 연결할 수 있게 해주는 Java API (Java 에서 제공하는 코드)
		 * => java.sql 패키지에 존재함!
		 */

		// Java 코드를 이용해 
		// EMPLOYEE 테이블에서 사번, 이름, 부서코드, 직급코드, 급여, 입사일 조회 후 이클립스 콘솔에 출력!
		...
		try {
			...
			String port = ":1521"; // DB 서버에 연결하기 위한 포트 번호
			String dbName = ":XE"; // 우리가 사용하는 DBMS 이름 (XE == eXpress Edition)
			
			// => jdbc:oracle:thin:@localhost:1521:XE
			
			String userName = "kh_pyj"; // 사용자 계정명
			...
			// 3. SQL 작성
			// 주의사항!
			// -> JDBC 코드에서 SQL 작성 시 세미콜론 작성하면 안됨!!
			// 작성할 시 "SQL 명령어가 올바르게 종료되지 않았습니다" 오류 발생!!
			...
			// 6. 조회 결과가 담겨있는 rs에
			// 커서(cursor)를 이용해서 1행씩 접근하여 각 행에 작성된 컬럼 값 얻어오기
			
			// boolean ResultSet.next() : 
			// 커서를 다음 행으로 이동 시킨 후 이동된 행의 값이 있으면 true, 없으면 false를 반환
			// 맨 처음 호출 시 1행부터 시작
			
			while(rs.next()) {
				// 자료형 ResultSet.get자료형(컬럼명 | 순서);
				// - 현재 행에서 지정된 컬럼의 값을 얻어와 반환
				// -> 지정된 자료형 형태로 값이 반환됨! (자료형 잘못 지정 시 예외 발생!)
				
				// [java]                      [db]
				// String			  CHAR, VARCHAR2, NVARCHAR2
				// int, long		 NUMBER (정수만 입력된 컬럼)
				// double, float        NUMBER (정수 + 실수)
				// java.sql.date 			   DATE	
			...
			// -> 자원반환 하지 않을 시 DB와 연결된 Connection이 그대로 남아 있어 
			// 다른 클라이언트(ex. Java 프로그램)와 추가적으로 연결되지 못하는 문제 발생할 수 있음!
			// => DBMS는 최대 Connection 수 개수 제한을 하고 있기 때문!
			
			try {
				// conn, stmt, rs (만들어진 역순으로 close 수행 권장!!)
				if(rs != null) rs.close();
