...
public class JDBCExample2 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 5. Statement 객체를 이용하여 SQL 결과(ResultSet) 반환 받아오기
			// Statement.executeQuery() : SELECT 실행, RESULT SET 반환
			// Statement.excuteUpdate() : DML 실행, 결과 행의 개수 반환 (int)
			rs = stmt.executeQuery(sql);
			
			// 6. 조회 결과가 담긴 Result Set에 커서를 이용해 한 행식 접근하여 컬럼 값 가져오기
			// => Result Set에 1행 이상이 있을 것으로 예상되는 경우에 반복문 사용!
